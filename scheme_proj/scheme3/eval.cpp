...
bool isQuoted(Expression *expr) { return isTaggedList(expr, Expression::quote_symbol()); }
Expression* textOfQuote(Expression *expr) { return cadr(expr); }

bool isAssign(Expression *expr) { return isTaggedList(expr,Expression::set_symbol()); }
Expression* assignmentVar(Expression *expr) { return cadr(expr); }
Expression* assignmentValue(Expression *expr) { return caddr(expr); }

bool isDefine(Expression *expr) { return isTaggedList(expr,Expression::define_symbol()); }
...
Expression* makeIf(Expression *predicate, Expression *consequent, Expression *alternative) {
    return cons(Expression::if_symbol(), cons(predicate, cons(consequent, cons(alternative, Expression::empty_list()))));
}
bool isIf(Expression *expr) { return isTaggedList(expr,Expression::if_symbol()); }
...
Expression* makeLambda(Expression *params, Expression *body) { return cons(Expression::lambda_symbol(), cons(params, body)); };
bool isLambda(Expression *expr) { return isTaggedList(expr, Expression::lambda_symbol()); }
Expression* lambdaParams(Expression *expr) { return cadr(expr); }
Expression* lambdaBody(Expression *expr) { return cddr(expr); }

Expression* makeBegin(Expression *expr) { return cons(Expression::begin_symbol(), expr); }
bool isBegin(Expression *expr) { return isTaggedList(expr, Expression::begin_symbol()); }
...
bool isCond(Expression *expr) { return isTaggedList(expr, Expression::cond_symbol()); }
...
bool isCondElseClause(Expression *clause) { return condPredicate(clause) == Expression::else_symbol(); }
...
bool isLet(Expression *expr) { return isTaggedList(expr, Expression::let_symbol()); }
...
Expression* bindingsParams(Expression *bindings) {
    return isEmptyList(bindings) ? Expression::empty_list() : cons(bindingParam(car(bindings)), bindingsParams(cdr(bindings)));
}
Expression* bindingsArgs(Expression *bindings) {
    return isEmptyList(bindings) ? Expression::empty_list() : cons(bindingArg(car(bindings)), bindingsArgs(cdr(bindings)));
}
...
bool isAnd(Expression *expr) { return isTaggedList(expr, Expression::and_symbol()); }
Expression* andTests(Expression *expr) { return cdr(expr); }
bool isOr(Expression *expr) { return isTaggedList(expr, Expression::or_symbol()); }
...
Expression* listOfValues(Expression *expr, Expression *env) {
    return (noOperands(expr)) ? Expression::empty_list() :
...
Expression* evalAssignment(Expression *expr, Expression *env) {
    theEnv::setVarValue(assignmentVar(expr), eval(assignmentValue(expr), env), env);
    return Expression::ok_symbol();
}
Expression* evalDefinition(Expression *expr, Expression *env) {    
    theEnv::defVar(definitionVar(expr), eval(definitionValue(expr), env), env);
    return Expression::ok_symbol();
}
...
tailcall:
    ...
    if (isSelfEval(expr)) return expr;
    else if (isVar(expr)) return theEnv::lookupVarValue(expr, env);
    ...
    else if (isLambda(expr)) { return new Expression(lambdaParams(expr), lambdaBody(expr), env); }
        ...
        if (isPrimProc(proc) && proc->data.primitive.func == evalProc) {
            expr = evalExpr(args);
            env = evalEnv(args);
            goto tailcall;
        }
        if (isPrimProc(proc) && proc->data.primitive.func == applyProc) {
            proc = applyOperator(args);
            args = applyOperands(args);
        }

        if (isPrimProc(proc)) return (proc->data.primitive.func)(args);
        else if (isCompProc(proc)) {
            env = theEnv::extendEnv(proc->data.compound.params, args, proc->data.compound.env);
            expr = makeBegin(proc->data.compound.body);
            goto tailcall;
