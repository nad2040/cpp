...
Expression* theEnv::lookupVarValue(Expression *var, Expression *env) {
    ...
    std::cerr << "unbound variable, " << var->data.symbol.val << '\n';
    exit(1);
}
...
Expression* theEnv::setupEnv() {
    Expression* empty_list = Expression::empty_list();
    return extendEnv(empty_list, empty_list, empty_list);
}

void theEnv::populateEnv(Expression* env) {
#define addSchemeProc(scheme_name, name) defVar(Expression::makeSymbol(scheme_name),new Expression(name), env)
...
Expression* theEnv::interactionEnvProc(Expression *args) { return global_env(); }
