...
struct Atom {
    ...
    std::variant<mybool, char, int, std::string> value_; //with bool and int together it cause crash
...
void display(const Atom& a) {
    switch(a.atomType_) {
        ...
        case Atom::BOOL: std::cout << "BOOL:" << std::get<mybool>(a.value_).b << '\n'; return;
...
int main() {
    ...
    atoms.push_back({Atom::BOOL, (mybool){true}});
