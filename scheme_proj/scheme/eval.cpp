...
Expression* makeIf(Expression *predicate, Expression *consequent, Expression *alternative) {
    return cons(Symbol::if_symbol(), cons(predicate, cons(consequent, cons(alternative, Expression::empty_list()))));
}
...
tailcall:
    ...
    if (isSelfEval(expr)) return expr;
        ...
        if (isPrimProc(proc) && proc->get<Atom>().get<Primitive>() == evalProc) {
            expr = evalExpr(args);
            env = evalEnv(args);
            goto tailcall;
        }
        if (isPrimProc(proc) && proc->get<Atom>().get<Primitive>() == applyProc) {
            proc = applyOperator(args);
            args = applyOperands(args);
        }

        if (isPrimProc(proc)) return (proc->get<Atom>().get<Primitive>())(args);
        else if (isCompProc(proc)) {
            env = theEnv::extendEnv(proc->get<Atom>().get<Compound>().params, args, proc->get<Atom>().get<Compound>().env);
            expr = makeBegin(proc->get<Atom>().get<Compound>().body);
            goto tailcall;
